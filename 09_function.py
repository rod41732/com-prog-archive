...
def solve_09_L4():
    ...
    def solvable(t):
        ...
        if n%2 == 1 and inv%2 == 0 or \
        n%2 == 0 and inv%2 != zero_pos%2:
            return True
        else:
            return False
...
def solve_09_P4():
    ...
    def minor(m, row, col):
        size = len(m)
        return [[m[i][j] for j in range(size) if j!= col] \
...
def solve_09_P5():
    def substr2num(s):
        pair=[['CM',900],['CD',400],['XC',90],['XL',40],['IX',9],['IV',4]]
        single=[['M',1000],['D',500],['C',100],['L',50],['X',10],['V',5],['I',1]]
        for roman, num in pair:
            if s == roman:
                return (num, 2)
        for roman, num in single:
            if s[:1] == roman:
                return (num, 1)
...
def solve_09_P8():
    def isSet(c1, c2, c3):
        ...
        # res = zip(c1, c2, c3) # [(col1, col2, col3), (cnt1, cnt2, cnt3), ..]
        # res = [set(c) for c in res]
        # for c in res
